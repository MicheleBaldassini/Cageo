# -*- coding: utf-8 -*-
"""Match canonically and export each input's nearest real simulation value."""
...
def match_generated_combinations_to_simulations(table_path, source_real, matching_real):
    ...
    choose the closest value from the entire real-simulation dataset separately
    for every input column. Targets come from the lowest-FoS matched row.
    """
    ...
    # Each column has one global pool of observed simulation values. Matching
    # and choosing the lowest-FoS row are separate from this output lookup.
    ...
    for wildcard_columns in wildcard_groups:
        ...
        for table_idx in rows_by_wildcards[wildcard_columns]:
            ...
            # independently takes the globally closest observed column value.
            ...
            for column in feature_columns:
                ...
                replacement = closest_real_value(real_candidates, column, canonical_value)
                if replacement is None and to_numeric_match_value(canonical_value) is None:
                    ...
                    replacement = closest_real_value(real_candidates, column, inferred)
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path(DATASET_DIR) / "filtered_no_canonical")
