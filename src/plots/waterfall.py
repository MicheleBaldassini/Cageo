"""Render customized SHAP waterfall plots."""
...
# of the bar...
def waterfall(shap_values, ax, max_display=20, show=True):
    ...
    if feature_names is None:
        feature_names = np.array([str(i) for i in range(len(values))])
    ...
    plt.gcf().set_size_inches(8, num_features * row_height + 2.5)
    ...
    for i in range(num_individual):
        ...
        # -------------------------------------------------------------
        # Apply custom formatting for k_sat and Tr.
        # -------------------------------------------------------------
        if features is None:
            yticklabels[rng[i]] = feature_names[order[i]]
        else:
            feat_name = str(feature_names[order[i]])
            feat_val = features[order[i]]

            if np.issubdtype(type(feat_val), np.number):
                val_float = float(feat_val)
                name_lower = feat_name.lower()

                # print(name_lower)

                # Format K_sat in scientific notation.
                if "k_{sat}" in name_lower:
                    str_val = f"{val_float:.0e}".replace("e-0", "e-")

                # Display a zero return period without decimals.
                elif "t_r" in name_lower and abs(val_float) < 1e-6:
                    str_val = "-"

                # Use standard formatting for all remaining features.
                else:
                    str_val = format_value(val_float, "%0.03f")

                yticklabels[rng[i]] = str_val + " = " + feat_name
            else:
                yticklabels[rng[i]] = str(feat_val) + " = " + feat_name
        # -------------------------------------------------------------
        # FINE MODIFICA
        # -------------------------------------------------------------
    ...
    for i in range(len(pos_inds)):
        ...
        if pos_low is not None and i < len(pos_low):
            ax.errorbar(
                ...
                ecolor=light_colors.red_rgb,
            )

        txt_obj = ax.text(
            pos_lefts[i] + 0.5 * dist,
            pos_inds[i],
            format_value(pos_widths[i], "%+0.03f"),
    ...
    for i in range(len(neg_inds)):
        ...
        if neg_low is not None and i < len(neg_low):
            ax.errorbar(
                ...
                ecolor=light_colors.blue_rgb,
            )

        txt_obj = ax.text(
            neg_lefts[i] + 0.5 * dist,
            neg_inds[i],
            format_value(neg_widths[i], "%+0.03f"),
            horizontalalignment="center",
            verticalalignment="center",
            color="white",
            fontsize=fontsize,
    ...
    tick_labels[0].set_transform(
        ...
        + matplotlib.transforms.ScaledTranslation(-50 / 72.0, 0, fig.dpi_scale_trans)
    )
    ...
    tick_labels[0].set_transform(
        tick_labels[0].get_transform()
        + matplotlib.transforms.ScaledTranslation(-80 / 72.0, 0, fig.dpi_scale_trans)
    )
    tick_labels[1].set_transform(
        tick_labels[1].get_transform()
        + matplotlib.transforms.ScaledTranslation(
            -25 / 72.0, -1 / 72.0, fig.dpi_scale_trans
        )
    )
    ...
    for i in range(num_features, 2 * num_features):
        tick_labels[i].set_bbox(dict(facecolor="white", edgecolor="none", pad=1))
